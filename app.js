...
const appVersion="Release Candidate 1";
...
function home(){
 ...
 let o=overall();
 app.innerHTML=`<img src="banner.jpg" class="banner">
...
function openSubject(code){if(!profileData())return profile();let s=SUBJECT_DATA[code],n=s.questions.length,free=Math.min(100,n),premium=Math.max(0,n-100);app.innerHTML=`${profileSummary()}<section class="card"><span class="badge">${s.paper}</span><h1>${code} — ${esc(s.name)}</h1><p>${n} valid scorable MCQs extracted from the supplied bank.</p></section><section class="feature-grid"><article class="card mode-card" onclick="timerSetup('${code}','free')"><span class="badge">Free</span><h3>Random ${free} Questions</h3><p>Student chooses the timer • minimum 20 minutes</p></article><article class="card mode-card" onclick="premiumLogin('${code}')"><span class="badge">Premium</span><h3>${premium} Remaining Questions</h3><p>Student chooses the timer • minimum 20 minutes</p></article><article class="card mode-card" onclick="premiumLogin('${code}','mock')"><span class="badge">Full Mock</span><h3>All ${n} Questions</h3><p>Student chooses the timer • premium access</p></article></section>`}
...
function review(){app.innerHTML=`<section class="card"><h1>Submitted Answer Review</h1><p>Correct answers are shown only for attempted questions.</p><div class="review-list">${lastResult.questions.map((q,i)=>{let u=lastResult.answers[i];if(u===null)return`<article class="card"><h3>${q.id}. ${esc(q.q)}</h3><div class="not-attempted"><b>Not Attempted</b><br>Correct answer hidden.</div></article>`;return`<article class="card"><h3>${q.id}. ${esc(q.q)}</h3><div class="option ${u===q.answer?"correct":"wrong"}"><b>Your answer:</b> ${String.fromCharCode(65+u)}. ${esc(q.options[u])}</div><div class="option correct"><b>Correct answer:</b> ${String.fromCharCode(65+q.answer)}. ${esc(q.options[q.answer])}</div></article>`}).join("")}</div><button class="btn" onclick="home()">Home</button></section>`}
...
function courseExpansionGuide(){
 app.innerHTML=`<section class="card"><span class="badge">Expandable Courses</span>
 <h1>Add More Courses Later</h1>
 <p>The website now uses one central <code>COURSE_CATALOG</code> list. New courses and levels can be added without redesigning the homepage.</p>
 <div class="not-attempted"><b>Current launch scope:</b><br>CMA Foundation, Intermediate and Final<br>CA Intermediate and Final</div>
 <p style="margin-top:14px">When another course is added later, only its course record and question bank need to be inserted.</p>
 <button class="btn ghost" onclick="home()">Back Home</button></section>`;
}
...
function adminLogin(){
 app.innerHTML=`<section class="card admin-login-card">
  ...
  <h1>Admin Login</h1>
  <p>Enter the admin code to manage premium access.</p>
...
function checkAdmin(){
 if(ac.value===ADMIN_CODE)adminPanel();
 else alert("Incorrect admin code.");
}
function adminPanel(){
 app.innerHTML=`<section class="card">
  <span class="badge">Admin Panel</span>
  <h1>Premium Access Settings</h1>
  <label>Current premium code</label>
  <input class="input" id="newPremiumCode" value="${esc(getPremiumCode())}" autocomplete="off">
  <p class="small">Students must enter this code to open premium tests and full mocks.</p>
  <div class="actions" style="margin-top:14px">
   <button class="btn gold" onclick="savePremiumCode()">Save New Code</button>
   <button class="btn ghost" onclick="courseExpansionGuide()">Course Setup</button>
...
function savePremiumCode(){
 const code=newPremiumCode.value.trim();
 if(code.length<4){
  adminStatus.innerHTML=`<div class="not-attempted" style="margin-top:12px">Use at least 4 characters.</div>`;
  return;
 }
 setPremiumCode(code);
 adminStatus.innerHTML=`<div class="success-note" style="margin-top:12px"><b>Saved.</b> The new premium code is active on this device.</div>`;
}
